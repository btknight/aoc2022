...
def sig_strength():
    ...
    while True:
        if working_on is None:
            print(f'fetching instruction for new cycle')
            try:
                line = next(rl)
            except StopIteration:
                break
            print(f'new instruction: {line}')
            if 'addx' in line:
                cmd, arg = line.split(' ')
        ...
        if working_on is not None and cycle == working_on[0]:
            print(f'Processing {working_on} completion')
...
sum = 0
for sig in sig_strength():
    print(f'sig: {sig}')
    sum += sig

print(f'Sum of signal strength: {sum}')
